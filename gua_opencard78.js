/*
12.7~12.12 联合开卡 [gua_opencard78.js]
新增开卡脚本
一次性脚本

1.邀请一人20豆(有可能没有豆
2.开13张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku78]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw78]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw78="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 12.7~12.12 联合开卡 (https://3.cn/-103E6rmr)]
...
[task_local]
#12.7~12.12 联合开卡
47 5 7-12 12 * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_opencard78.js, tag=12.7~12.12 联合开卡, enabled=true
...
const $ = new Env('12.7~12.12 联合开卡-78');
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku78 ? process.env.guaopencard_addSku78 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku78') ? $.getdata('guaopencard_addSku78') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard78 ? process.env.guaopencard78 : `${guaopencard}`) : ($.getdata('guaopencard78') ? $.getdata('guaopencard78') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait78 ? process.env.guaopenwait78 : `${guaopenwait}`) : ($.getdata('guaopenwait78') ? $.getdata('guaopenwait78') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw78 ? process.env.guaopencard_draw78 : guaopencard_draw) : ($.getdata('guaopencard_draw78') ? $.getdata('guaopencard_draw78') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard78]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.userId = '10299171'
  $.actId = '2a6e003c0510_1207'
  $.MixNicks = ''
  $.inviteNick = 'jJngxInVOgsjftAvjP1dgF4tLNYA4seuA67MOIYQxEk3Vl9+AVo4NF+tgyeIc6A6kdK3rLBQpEQH9V4tdrrh0w=='
  console.log(`活动地址:https://jinggengjcq-isv.isvjcloud.com/fronth5/#/pages/unitedCardNew20211207/unitedCardNew20211207?actId=2a6e003c0510_1207`)
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku78]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw78]为"3" 3为次数');
