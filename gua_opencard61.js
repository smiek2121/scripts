/*
11.1~11.11 联合开卡 [gua_opencard61.js]
新增开卡脚本
一次性脚本
...
2.开18张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku61]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw61]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw61="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 11.1~11.11 联合开卡 (https://3.cn/103jrK-Lc)]
...
[task_local]
#11.1~11.11 联合开卡
47 12 1-11 10 * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_opencard61.js, tag=11.1~11.11 联合开卡, enabled=true
...
const $ = new Env('11.1~11.11 联合开卡-61');
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku61 ? process.env.guaopencard_addSku61 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku61') ? $.getdata('guaopencard_addSku61') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard61 ? process.env.guaopencard61 : `${guaopencard}`) : ($.getdata('guaopencard61') ? $.getdata('guaopencard61') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait61 ? process.env.guaopenwait61 : `${guaopenwait}`) : ($.getdata('guaopenwait61') ? $.getdata('guaopenwait61') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw61 ? process.env.guaopencard_draw61 : guaopencard_draw) : ($.getdata('guaopencard_draw61') ? $.getdata('guaopencard_draw61') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard61]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.userId = '10299171'
  $.actId = 'a8e0ad103d1e477ab984_11017'
  $.MixNicks = ''
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku61]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw61]为"3" 3为次数');
