...
2.开17张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku55]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw55]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw55="3"
填非数字会全都抽奖
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku55 ? process.env.guaopencard_addSku55 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku55') ? $.getdata('guaopencard_addSku55') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard55 ? process.env.guaopencard55 : `${guaopencard}`) : ($.getdata('guaopencard55') ? $.getdata('guaopencard55') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw55 ? process.env.guaopencard_draw55 : guaopencard_draw) : ($.getdata('guaopencard_draw55') ? $.getdata('guaopencard_draw55') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard55]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.userId = '10299171'
  $.actId = '17ba854eb84a4b228834e134_1025'
  $.MixNicks = ''
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku55]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw55]为"3" 3为次数');
