/*
12.12~12.20 狂欢购 [gua_opencard83.js]
新增开卡脚本
一次性脚本
...
3.关注10豆 
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard_addSku83]为"true"
5.浏览店铺1豆/个
6.浏览商品1豆/个
...
All变量适用
————————————————
入口：[ 12.12~12.20 狂欢购 (https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity?activityId=dzcjlhkk20211212&shareUuid=f9cf28a4e55043b9a1c110cc9fa2326d)]

请求太频繁会被黑ip
过10分钟再执行

cron:47 3 13-20/2 12 *
============Quantumultx===============
[task_local]
#12.12~12.20 狂欢购
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku83 ? process.env.guaopencard_addSku83 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku83') ? $.getdata('guaopencard_addSku83') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard83 ? process.env.guaopencard83 : `${guaopencard}`) : ($.getdata('guaopencard83') ? $.getdata('guaopencard83') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait83 ? process.env.guaopenwait83 : `${guaopenwait}`) : ($.getdata('guaopenwait83') ? $.getdata('guaopenwait83') : `${guaopenwait}`);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard83]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.activityId = "dzcjlhkk20211212"
  $.shareUuid = "f9cf28a4e55043b9a1c110cc9fa2326d"
  ...
  try {
    ...
    if(!$.addSku && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard_addSku83]为"true"');
      }
    }
    $.runFalag = true
    $.log("浏览店铺: " + $.toShop)
