...
!(async () => {
  ...
  $.projectId = "vKCsPTgzFyAUzJzRhCTqgk6dRpk"
  $.assignmentId = "3895bnp5HkN49qd2mBJGVb4bADP3"
  $.helpType = 2
  $.type = 2
  if($.temp.length > 0){
  ...
  if(errorMsg.length > 0){
    allMessage += `异常的有账号：${errorMsg.join('、')}\n`
  }
  if(errorMsgs.length > 0){
    allMessage += `火爆的有账号：${errorMsgs.join('、')}\n`
  }
  if(allMessage){
    $.msg($.name, ``, `${allMessage}`);
  ...
  try {
    ...
    for(let i in $.list || []){
      if($.list[i]){
        ...
        if($.list[i].name.indexOf('分享活动') > -1) continue
        await takePostRequest('查询任务');
        // console.log($.Task)
        if(Object.getOwnPropertyNames($.Task).length > 0){
          ...
          if($.Task.status+"" === "0"){
            ...
            if($.Task.waitDuration > 0){
              await takePostRequest('做任务_accept');
...
async function takePostRequest(type) {
  ...
  switch (type) {
    case '查询':
      url = `https://api.m.jd.com/interactive_rewardInfo?functionId=interactive_rewardInfo&appid=contenth5_common&body={"encryptProjectPoolId":"3vx2VzqP4Fs79GQ9tcoSDzkiMBEN"}&client=wh5`;
      body = ``;
      break;
    case '查询任务':
      ...
      break;
    case '助力':
      url = `https://api.m.jd.com/interactive_info?functionId=interactive_info&appid=contenth5_common&body=[{"type":"2","projectId":"${$.projectId}","assignmentId":"${$.assignmentId}","doneHide":false,"helpType":"${$.helpType}","itemId":"${$.itemId}"}]&client=wh5`;
      body = ``;
      break;
    case '做任务_done':
    case '做任务_accept':
    case 'aha_card_list':
      if(type == '做任务_done'){
        functionId = 'interactive_done'
      }else if(type == '做任务_accept'){
      ...
      if($.type == 11 || $.type == 10){
        ...
        url = `https://api.m.jd.com/${functionId}?functionId=${functionId}&appid=contenth5_common&body={"projectId":"${$.projectId}","assignmentId":"${$.assignmentId}","type":"${$.type}"${bodys}}&client=wh5`;
      }else{
        url = `https://api.m.jd.com/${functionId}?functionId=${functionId}&appid=contenth5_common&body={"projectId":"${$.projectId}","assignmentId":"${$.assignmentId}","type":"${$.type}","itemId":"${$.itemId}"${bodys}}&client=wh5`;
      }
      body = ``;
      if($.type == 9){
...
function getList1() {
  return new Promise(resolve => {
    let opts = {
      url:`https://prodev.m.jd.com/mall/active/3Q4roJtRWzTom2stXm6wEJTH8H2w/index.html`,
          ...
          if(Object.getOwnPropertyNames(list).length > 0){
            for(let i of list.activityData.floorList || []){
              if(i.boardParams && i.boardParams.projectCode && i.boardParams.taskCode){
                let type = 1
                if(i.boardParams.taskCode == "4VfjYLbkAgxC55Yp1dXUPU7RHR2t") type = 5
                if(i.boardParams.taskCode == "uP1wnfE7CgmTe3zJ4Wmdrgqy7CN") type = 10
                if(i.boardParams.taskCode == "2jYXWJX9CJD8Jy8VjPdXMqXabaou") type = 1
                if(i.boardParams.taskCode == "36m4VjdZJkEoPUXvmy62iV1km8z") type = 1
                if(i.boardParams.taskCode == "3895bnp5HkN49qd2mBJGVb4bADP3") type = 2
...
function getPostRequest(url,body) {
  let headers =  {
    ...
    "Referer": `https://prodev.m.jd.com/mall/active/3Q4roJtRWzTom2stXm6wEJTH8H2w/index.html`,
    "User-Agent": `${UA || $.UA}` ,
  }
  // console.log(headers)
