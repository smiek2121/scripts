/*
12.18~12.24 联合开卡 [gua_opencard88.js]
新增开卡脚本
一次性脚本

1.邀请一人10豆(有可能没有豆
2.开16张 成功开1张 可能获得10京豆
3.关注20京豆 获得1次抽奖
4.加购30京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku88]为"true"


5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw88]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw88="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 12.18~12.24 联合开卡 (https://3.cn/-103LNFwO)]

请求太频繁会被黑ip
过10分钟再执行


cron:30 3 22-24/2 12 *
============Quantumultx===============
[task_local]
#12.18~12.24 联合开卡
30 3 22-24/2 12 * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_opencard88.js, tag=12.18~12.24 联合开卡, enabled=true
...
const $ = new Env('12.18~12.24 联合开卡-88');
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku88 ? process.env.guaopencard_addSku88 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku88') ? $.getdata('guaopencard_addSku88') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard88 ? process.env.guaopencard88 : `${guaopencard}`) : ($.getdata('guaopencard88') ? $.getdata('guaopencard88') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait88 ? process.env.guaopenwait88 : `${guaopenwait}`) : ($.getdata('guaopenwait88') ? $.getdata('guaopenwait88') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw88 ? process.env.guaopencard_draw88 : guaopencard_draw) : ($.getdata('guaopencard_draw88') ? $.getdata('guaopencard_draw88') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard88]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku88]为"true"');
    }
    await takePostRequest('activity_load');
