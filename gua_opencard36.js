...
开22卡 每个卡10京豆 (如果在别的地方开过卡 则在这不能开卡 故没有京豆
3个日常任务 有关注、加购 (每次1京豆 关注和加购可以多次
(默认不加购 如需加购请设置环境变量[guaopencard_addSku36]为"true"

默认脚本不执行
如需执行脚本请设置环境变量
做任务
guaopencardRun36="true"
开卡
guaopencard36="true"
...
————————————————
入口：[ 生活有范 大牌狂欢 (https://prodev.m.jd.com/mall/active/8R1TdWWLuC9o6uRvnvzzMLnut6F/index.html)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku36 ? process.env.guaopencard_addSku36 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku36') ? $.getdata('guaopencard_addSku36') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard36 ? process.env.guaopencard36 : `${guaopencard}`) : ($.getdata('guaopencard36') ? $.getdata('guaopencard36') : `${guaopencard}`);
...
guaopencardRun = $.isNode() ? (process.env.guaopencardRun36 ? process.env.guaopencardRun36 : `${guaopencardRun}`) : ($.getdata('guaopencardRun36') ? $.getdata('guaopencardRun36') : `${guaopencardRun}`);
...
!(async () => {
  ...
  if(guaopencard+"" != "true"){
    console.log('如需开卡请设置环境变量[guaopencard36]为"true"')
  }
  if(guaopencardRun+"" != "true"){
    console.log('如需做任务请设置环境变量[guaopencardRun36]为"true"')
  }
  if(guaopencard+"" != "true" && guaopencardRun+"" != "true"){
    return
  }
  $.activeID = '8R1TdWWLuC9o6uRvnvzzMLnut6F'
...
async function run() {
  try {
    ...
    let config = [
      {configCode:'8547f039d10a4faea2e60d1589f9c08b',configName:'生活有范 大牌狂欢'},
    ]
    for(let i in config){
      $.hotFlag = false
      let item = config[i]
      ...
      if($.task == '') message += `[${item.configName}] 任务获取失败，再次执行\n`
      if($.task.memberTask && $.task.memberTask.memberList){
        let msg = ''
        if(guaopencard+"" != "true"){
          msg = '如需开卡请设置环境变量[guaopencard36]为"true"'
        }
        ...
        for (let o in $.task.memberTask.memberList) {
          ...
          if($.oneTask.result == 0) await $.wait(parseInt(Math.random() * 1000 + 2000, 10))
          ...
          if($.oneTask.result == 1 || $.oneTask.result == 0) await $.wait(parseInt(Math.random() * 2000 + 2000, 10))
        }
      ...
      if($.task.dailyTask && $.task.dailyTask.taskList){
        for (let t in $.task.dailyTask.taskList) {
          ...
          if(guaopencard_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[guaopencard_addSku36]为"true"\n');
            ...
            for(n=2;c-- && !$.outActivity;n++){
              ...
              if($.oneTasks){
                ...
                if($.oneTasks.item.itemId == $.oneTask.item.itemId){
                  ...
                  $.cacheNum++;
                  if($.cacheNum > 3) {
                    console.log('请重新执行脚本，数据缓存问题');
                    message += "请重新执行脚本，数据缓存问题\n"
                    break
                  };
...
function getHtml() {
  return new Promise(resolve => {
    $.get({
      url: `https://prodev.m.jd.com/mall/active/${$.activeID}/index.html`,
      headers: {
...
async function getUA(){
  ...
  let arr = await Faker.getBody($.UA,`https://prodev.m.jd.com/mall/active/${$.activeID}/index.html`)
  $.fp = arr.fp
  await getEid(arr)
}
