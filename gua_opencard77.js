/*
12.1~12.12 联合开卡 [gua_opencard77.js]
新增开卡脚本
一次性脚本
...
2.开12张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku77]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw77]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw77="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 12.1~12.12 联合开卡 (https://3.cn/103-E4GKW)]
...
const $ = new Env('12.1~12.12 联合开卡-77');
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku77 ? process.env.guaopencard_addSku77 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku77') ? $.getdata('guaopencard_addSku77') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard77 ? process.env.guaopencard77 : `${guaopencard}`) : ($.getdata('guaopencard77') ? $.getdata('guaopencard77') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait77 ? process.env.guaopenwait77 : `${guaopenwait}`) : ($.getdata('guaopenwait77') ? $.getdata('guaopenwait77') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw77 ? process.env.guaopencard_draw77 : guaopencard_draw) : ($.getdata('guaopencard_draw77') ? $.getdata('guaopencard_draw77') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard77]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  console.log(`活动地址:https://jinggengjcq-isv.isvjcloud.com/fronth5/#/pages/unitedCardNew2021120103/unitedCardNew2021120103?actId=4fed35939fc348969bff_120103`)
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku77]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw77]为"3" 3为次数');
