/*
12.9~12.15 联合开卡 [gua_opencard80.js]
新增开卡脚本
一次性脚本
...
2.开13张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku80]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw80]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw80="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 12.9~12.15 联合开卡 (https://3.cn/-103FxWCS)]
...
[task_local]
#12.9~12.15 联合开卡
30 1 9-15 12 * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_opencard80.js, tag=12.9~12.15 联合开卡, enabled=true
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku80 ? process.env.guaopencard_addSku80 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku80') ? $.getdata('guaopencard_addSku80') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard80 ? process.env.guaopencard80 : `${guaopencard}`) : ($.getdata('guaopencard80') ? $.getdata('guaopencard80') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait80 ? process.env.guaopenwait80 : `${guaopenwait}`) : ($.getdata('guaopenwait80') ? $.getdata('guaopenwait80') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw80 ? process.env.guaopencard_draw80 : guaopencard_draw) : ($.getdata('guaopencard_draw80') ? $.getdata('guaopencard_draw80') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard80]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  console.log(`活动地址:https://jinggengjcq-isv.isvjcloud.com/fronth5/#/pages/unitedCardNew20211209/unitedCardNew20211209?actId=73hdkjydcg7uhu0ce8rt_1209`)
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku80]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw80]为"3" 3为次数');
