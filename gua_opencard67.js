/*
11.6~11.12 联合开卡 [gua_opencard67.js]
新增开卡脚本
一次性脚本
...
2.开17张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku67]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw67]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw67="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 11.6~11.12 联合开卡 (https://3.cn/10-3pnnrY)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku67 ? process.env.guaopencard_addSku67 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku67') ? $.getdata('guaopencard_addSku67') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard67 ? process.env.guaopencard67 : `${guaopencard}`) : ($.getdata('guaopencard67') ? $.getdata('guaopencard67') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait67 ? process.env.guaopenwait67 : `${guaopenwait}`) : ($.getdata('guaopenwait67') ? $.getdata('guaopenwait67') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw67 ? process.env.guaopencard_draw67 : guaopencard_draw) : ($.getdata('guaopencard_draw67') ? $.getdata('guaopencard_draw67') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard67]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku67]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw67]为"3" 3为次数');
